...
class ProduksiSir20ApiController extends Controller
{
        ...
        try {
            ...
            foreach ($bak_aktif_raw as $bak) {
                ...
                if ($stok_kemarin > 0.01) {
                    ...
                    // Tambahkan field yang dibutuhkan Java Android (Bug fix umur baris pertama)
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $bahanController = new BahanProsesApiController();
            $bahanController->syncChainData($request->tanggal_produksi);

            DB::commit();
            return response()->json(['success' => true, 'message' => 'Produksi berhasil disimpan & WIP terupdate.'], 201);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $bahanApi->syncChainData($newDate);
            if ($oldDate != $newDate) {
                $bahanApi->syncChainData($oldDate);
            }

            DB::commit();
            return response()->json(['success' => true, 'message' => 'Produksi API Berhasil Diperbarui.']);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    ...
    private function triggerUpdateMaturasi($namaRuang, $tanggal, $berat, $aksi)
    {
        ...
        if ($maturasi) {
            // 1. Update Log Pengolahan (Kolom diolah)
            $log = PengolahanMaturasi::firstOrCreate(
                ['id_maturasi' => $maturasi->id_maturasi, 'tgl_laporan' => $tanggal],
                ['masuk_hi' => 0, 'diolah' => 0, 'mutasi' => 0, 'keterangan' => 'Auto Produksi API']
            );
            ...
            // 2. 🔥 PERBAIKAN: Panggil Trait untuk Sinkronisasi Total
            // Ini akan mengupdate stok_akhir master, label CMP, dan status KOSONG
