...
class HasilUjiBokarExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles
{
    ...
    public function collection() {
        return HasilUjiLabBokar::when($this->startDate && $this->endDate, function($q) {
    ...
    public function headings(): array {
        ...
        return [
            ['LAPORAN UJI LAB BOKAR DITERIMA'],
            ['Periode: ' . $periode],
            [],
            ['Tanggal', 'Supplier', 'No Sampel', 'K3 (%)', 'Dirt (%)', 'Ash (%)', 'Po', 'Pa', 'PRI']
        ];
    }
    ...
    public function map($row): array {
        return [
            ...
            $row->suplier,
            $row->no_sampel,
            $row->k3, $row->dirt, $row->ask, $row->po, $row->pa, $row->pri
        ];
    }
