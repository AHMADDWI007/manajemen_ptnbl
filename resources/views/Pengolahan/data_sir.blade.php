    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid d-flex justify-content-between align-items-center">
                <h3 class="mb-0 text-success fw-bold">Data SIR (Gudang)</h3>
                <div>
                    {{-- HANYA 1 TOMBOL INPUT DATA (GABUNGAN) --}}
                    <button type="button" class="btn btn-primary btn-sm fw-bold mr-2" id="btnInputData">
            ...
            <div class="container-fluid">
                ...
                <div class="card shadow-sm mb-3">
                    <div class="card-body py-2">
                        <form method="GET" action="{{ route('data-sir.index') }}" class="form-inline justify-content-end">
                            <label class="mr-2">Tanggal:</label>
                            <input type="date" name="filter_tanggal" id="filter_tanggal" class="form-control form-control-sm mr-2" value="{{ $selected_date }}">
                            ...
                            <table class="table table-bordered table-striped mb-0">
                                ...
                                @foreach ($tabelIV as $item)
                                    <tr>
                                        ...
                                        <td class="text-right font-weight-bold text-primary">{{ number_format($item->masuk, 2, ',', '.') }}</td>
                                        <td class="text-right">{{ number_format($item->total, 2, ',', '.') }}</td>
                                        <td class="text-right">{{ number_format($item->prod_bln_lalu, 2, ',', '.') }}</td>
                                        <td class="text-right">{{ number_format($item->prod_sd_hi, 2, ',', '.') }}</td>
                                        <td class="text-right font-weight-bold text-danger">{{ number_format($item->pengiriman, 2, ',', '.') }}</td>
                                        <td class="text-right font-weight-bold">{{ number_format($item->saldo_akhir, 2, ',', '.') }}</td>
                                        <td class="text-center">-</td>
                                        <td class="text-center">
                                            {{-- TOMBOL AKSI LENGKAP --}}
                                            <div class="btn-group">
                                                {{-- UBAH BUTTON JADI TEKS 'Aksi' --}}
                                                <button type="button" class="btn btn-warning btn-sm dropdown-toggle font-weight-bold" data-toggle="dropdown">
                                                    Aksi
                                                </button>
                                ...
                                <tr class="row-jumlah">
                                    ...
                                    <td class="text-right">{{ number_format($tabelIV->sum('saldo_awal'), 2, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($tabelIV->sum('masuk'), 2, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($tabelIV->sum('total'), 2, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($tabelIV->sum('prod_bln_lalu'), 2, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($tabelIV->sum('prod_sd_hi'), 2, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($tabelIV->sum('pengiriman'), 2, ',', '.') }}</td>
                                    <td class="text-right">{{ number_format($tabelIV->sum('saldo_akhir'), 2, ',', '.') }}</td>
                                    <td class="text-right bg-warning font-weight-bold">{{ number_format($grandTotal ?? 0, 2, ',', '.') }}</td>
                                    <td></td>
                                </tr>
                                </tbody>
                            </table>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped mb-0">
                                ...
                                @foreach ($tabelVI as $item)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <div class="btn-group">
                                                {{-- UBAH BUTTON JADI TEKS 'Aksi' --}}
                                                <button type="button" class="btn btn-info btn-sm dropdown-toggle font-weight-bold" data-toggle="dropdown">
                                                    Aksi
                                                </button>
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    {{-- DETAIL --}}
                                ...
                                <tr class="row-jumlah">
                                    ...
                                    <td class="text-right">{{ number_format($tabelVI->sum('kg'), 2, ',', '.') }}</td>
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-2">
                            <label><small>Mutu Prima (Siap Jual)</small></label>
                            <input type="number" step="0.01" name="mutu_prima" id="in_mutu_prima" class="form-control" placeholder="0">
                        </div>
                        <div class="col-md-6 mb-2">
                            <label><small>PO / PRI Low</small></label>
                            <input type="number" step="0.01" name="po_pri" id="in_po_pri" class="form-control" placeholder="0">
                        </div>
                        <div class="col-md-6 mb-2">
                            <label><small>WhiteSpot (WS)</small></label>
                            <input type="number" step="0.01" name="ws" id="in_ws" class="form-control" placeholder="0">
                        </div>
                        <div class="col-md-6 mb-2">
                            <label><small>Kontaminasi</small></label>
                            <input type="number" step="0.01" name="kontaminasi" id="in_kontaminasi" class="form-control" placeholder="0">
                        </div>
                        <div class="col-md-6 mb-2">
                            <label><small>Repacking On Hold</small></label>
                            <input type="number" step="0.01" name="repacking" id="in_repacking" class="form-control" placeholder="0">
                        </div>
                    </div>
                    <hr>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea name="keterangan" id="inputKeterangan" class="form-control" rows="2" placeholder="Contoh: PTNB4"></textarea>
                    </div>
...
{{-- MODAL DETAIL IV & VI (Hanya untuk Display) --}}
<div class="modal fade" id="modalDetailIV" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-warning text-white">
                <h5 class="modal-title">Detail Gudang SIR</h5>
                <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <dl class="row mb-0">
                    <dt class="col-sm-4">Uraian</dt><dd class="col-sm-8" id="detUraianIV"></dd>
                    <dt class="col-sm-4">Masuk</dt><dd class="col-sm-8" id="detMasukIV"></dd>
                    <dt class="col-sm-4">Pengiriman</dt><dd class="col-sm-8" id="detPengirimanIV"></dd>
                    <dt class="col-sm-4">Keterangan</dt><dd class="col-sm-8" id="detKetIV"></dd>
                </dl>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="modalDetailVI" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title">Detail Data Mutu</h5>
                <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <dl class="row mb-0">
                    <dt class="col-sm-4">Uraian</dt><dd class="col-sm-8" id="detUraianVI"></dd>
                    <dt class="col-sm-4">Kg</dt><dd class="col-sm-8" id="detKgVI"></dd>
                    <dt class="col-sm-4">Pallet</dt><dd class="col-sm-8" id="detPalletVI"></dd>
                    <dt class="col-sm-4">Keterangan</dt><dd class="col-sm-8" id="detKetVI"></dd>
                </dl>
            </div>
        </div>
    </div>
...
<script>
    ...
    function bukaDetailIV(el) {
        $('#detUraianIV').text($(el).data('uraian'));
        $('#detMasukIV').text($(el).data('masuk'));
        $('#detPengirimanIV').text($(el).data('pengiriman'));
        $('#detKetIV').text($(el).data('ket'));
        $('#modalDetailIV').modal('show');
    }

    function bukaDetailVI(el) {
        $('#detUraianVI').text($(el).data('uraian'));
        $('#detKgVI').text($(el).data('kg'));
        $('#detPalletVI').text($(el).data('pallet'));
        $('#detKetVI').text($(el).data('ket'));
        $('#modalDetailVI').modal('show');
    }

    $(document).ready(function () {
        // TOMBOL INPUT BARU (FETCH API)
        $('#btnInputData').click(function() {
            var tanggal = $('#filter_tanggal').val();
            $('#modalTitle').text('Input Data Gudang & Mutu');
            $('#groupGudang').show(); // Tampilkan pilihan gudang
            $('#inputUraian').val('Di Gudang SIR'); 
            $('#inputKeterangan').val('');
        ...
        $(document).on('click', '.btn-edit-iv', function() {
            ...
            // Kosongkan mutu saat edit gudang agar tidak tertimpa
            $('#in_mutu_prima').val(''); $('#in_po_pri').val(''); $('#in_ws').val(''); $('#in_kontaminasi').val(''); $('#in_repacking').val('');
            $('#modalInputData').modal('show');
        });
        ...
        $(document).on('click', '.btn-edit-vi', function() {
            ...
            $('#groupGudang').hide(); // Sembunyikan gudang karena ini edit mutu spesifik
            $('#inputUraian').val(''); // Bypass required sementara, atau biarkan hidden
